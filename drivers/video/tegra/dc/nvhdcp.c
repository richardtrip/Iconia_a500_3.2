...
struct file* open_file(char *path,int flag,int mode)
{
	...
	ptr = filp_open(path, flag, mode);
	if (ptr) {
		return ptr;
	}

	pr_err("[hdcp] open file failed[%s,%d,%d]\n", path, flag, mode);
	return NULL;
}

void close_file(struct file *ptr)
{
	if (ptr) {
		filp_close(ptr, NULL);
		ptr = NULL;
...
int read_file(struct file *ptr, unsigned char *buf, unsigned int len)
{
	if (ptr && ptr->f_op && ptr->f_op->read) {
		ptr->f_pos = 0;
		return ptr->f_op->read(ptr , buf, len, &ptr->f_pos);
	}

	return -EIO;
}

int nvhdcp_read_file(struct tegra_nvhdcp_data *buf)
{
	struct file* ptr;
	int size;
	int res = 0;
	...
	ptr = open_file(buf->path, O_RDONLY, 0);
	if (!ptr) {
		pr_err("[hdcp] read file failed\n");
		res = -ENOENT;
		goto failed_open_file;
	}

	if ((size = read_file(ptr, buf->data, buf->size)) < 0) {
		pr_err("[hdcp] read file failed[%d]\n", size);
		res = -EIO;
		goto failed_read_file;
	}
                ...
                unsigned int cmd, unsigned long arg)
{
	...
	struct tegra_nvhdcp_data *buf = NULL;
	int e = -ENOTTY;
	...
	case TEGRAIO_NVHDCP_READ_FILE:
		buf = kmalloc(sizeof(*buf), GFP_KERNEL);
		if (copy_from_user(buf, (void __user *)arg, sizeof(*buf))) {
			pr_err("[hdcp] get variable from user failed\n");
			e = -EFAULT;
			goto kfree_buf;
		}
		e = nvhdcp_read_file(buf);
		if (e || copy_to_user((void __user *)arg, buf, sizeof(*buf))) {
			pr_err("[hdcp] set variable to user failed\n");
			e = -EFAULT;
			goto kfree_buf;
		}
		kfree(buf);
