...
static int nvhost_channelopen(struct inode *inode, struct file *filp)
{
	...
	if (!ch) {
		pr_err("%s: failed to nvhost_getchannel\n", __func__);
		return -ENOMEM;
	}
	...
	if (!priv) {
		...
		pr_err("%s: failed to alloc userctx\n", __func__);
		return -ENOMEM;
	}
	...
	if (IS_ERR(priv->gather_mem)) {
		pr_err("%s: failed to alloc gather memory\n", __func__);
		goto fail;
	}
	...
	if (ch->ctxhandler.alloc) {
		...
		if (!priv->hwctx) {
			pr_err("%s: failed to alloc hwctx\n", __func__);
			goto fail;
		}
	}

	priv->gathers = (struct nvhost_op_pair *)nvmap_mmap(priv->gather_mem);
	if (priv->gathers == NULL) {
		pr_err("%s: failed to mmap gather\n", __func__);
		goto fail;
	}
